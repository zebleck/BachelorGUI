...
class RatioBuilder:
    ...
    def post_904IC(self, data, i, tail_mat_cup, ThH_plus, blk, datablkm, UH_plus, cps, yield_U, R34_33, tail_mat,
        ...
        data33 = data[:, self.layout['233U']] - data[:, self.layout['238U']] * tail_mat_cup[3] - data[:, self.layout['232Th']] * ThH_plus - blk * datablkm[i, 0]  # U233

        data34 = data[:, self.layout['234U']] - UH_plus * data33 * cps * yield_U - data33 * R34_33 * cps * yield_U - data[:, self.layout['238U']] * \
                 tail_mat[4] * cps * yield_U - blk * datablkm[i, 1]  # U234

        data35 = (data[:, self.layout['235U']] - (data[:, self.layout['234U']] * UH_plus) / (cps * yield_U)) - data33 * R35_33 - data[:, self.layout['238']] * tail_mat_cup[
            5] - blk * datablkm[i, 2]  # U235

        data36 = data[:, self.layout['236U']] - UH_plus * data[:, self.layout['235U']] - data[:, self.layout['238U']] * tail_mat_cup[6] - blk * datablkm[i, 3]  # U236

        data38 = data[:, self.layout['238U']] - blk * datablkm[i, 4]  # U238

        data29 = data[:, self.layout['229Th']] - data[:, self.layout['238U']] * tail_mat[0] * cps * yield_U - tail_mat_th[0] * cps * yield_Th * data[:, self.layout['232Th']] - blk * \
                 datablkm[i, 7] - data[:, self.layout['238U']] * slope229Correction  # Th229

        data30 = data[:, self.layout['230Th']] - ThH_plus * data29 - data29 * R30_29 - data[:, self.layout['238U']] * tail_mat[1] * cps * yield_U - \
                 tail_mat_th[1] * cps * yield_Th * data[:, self.layout['232Th']] - blk * datablkm[i, 5]  # Th230 U

        data32 = data[:, self.layout['232Th']] - data[:, self.layout['238U']] * tail_mat_cup[2] - blk * datablkm[i, 6]  # Th232
    ...
    def post_90IC(self, data, i, tail_mat_cup, ThH_plus, blk, gain13, datablkm, UH_plus, cps, yield_U, R34_33, tail_mat,
        ...
        data34 = gain13 * data[:, self.layout['234U']] - UH_plus * data33 - data33 * R34_33 - data[:, self.layout['238U']] * tail_mat_cup[4] - blk * \
                 datablkm[i, 1] / (cps * yield_U)  # U234

        data35 = (data[:, self.layout['235U']] - data[:, self.layout['234U']] * UH_plus * gain13) - data33 * R35_33 - data[:, self.layout['238U']] * tail_mat_cup[5] - blk * \
                 datablkm[i, 2]  # U235
        ...
        data29 = data[:, 8] - data[:, self.layout['238U']] * tail_mat[0] * cps * yield_U - tail_mat_th[0] * cps * yield_Th * data[:, self.layout['232Th']] - blk * \
                 datablkm[i, 7] - data[:, self.layout['238U']] * slope229Correction  # Th229

        data30 = data[:, self.layout['230Th']] - ThH_plus * data29 - data29 * R30_29 - data[:, self.layout['238U']] * tail_mat[1] * cps * yield_U - \
                 tail_mat_th[1] * cps * yield_Th * data[:, self.layout['232Th']] - blk * datablkm[i, 5]  # Th230
    ...
    def post_0IC(self, data, i, tail_mat_cup, ThH_plus, blk, gain13, datablkm, UH_plus, cps, yield_U, R34_33, tail_mat,
        ...
        data29 = data[:, self.layout['229Th']] - data[:, self.layout['238U']] * tail_mat_cup[0] - tail_mat_th_cup[0] * data[:, self.layout['232Th']] - blk * datablkm[
            i, 7] - data[:, self.layout['238U']] * slope229Correction / cps  # Th229

        data30 = data[:, self.layout['230Th']] - ThH_plus * data29 * cps * yield_Th - data29 * R30_29 * cps * yield_Th - data[:, self.layout['238U']] * \
                 tail_mat[1] * cps * yield_U - tail_mat_th[1] * cps * yield_Th * data[:, self.layout['232Th']] - blk * datablkm[
                     i, 5]  # Th230

        data32 = data[:, self.layout['232Th']] - data[:, self.layout['238U']] * tail_mat_cup[2] - blk * datablkm[6, i]  # Th232
    ...
    def post_allcup(self, data, i, tail_mat_cup, ThH_plus, blk, gain13, datablkm, UH_plus, cps, yield_U, R34_33,
        ...
        data29 = data[:, self.layout['229Th']] - data[:, self.layout['238U']] * tail_mat_cup[0] - tail_mat_th_cup[0] * data[:, self.layout['232Th']] - blk * datablkm[
            i, 7] / cps - data[:, self.layout['238U']] * slope229Correction / cps  # Th229

        data30 = gain13 * data[:, self.layout['230Th']] - ThH_plus * data29 - data29 * R30_29 - data[:, self.layout['238U']] * tail_mat_cup[1] - \
                 tail_mat_th_cup[1] * data[:, self.layout['232Th']] - blk * datablkm[i, 5] / cps  # Th230
